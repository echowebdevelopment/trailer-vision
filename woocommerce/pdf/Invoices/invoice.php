		...
		<?php foreach ($this->get_order_items() as $item_id => $item): ?>
			<tr class="<?php echo esc_html($item['row_class']); ?>">
				...
				<td class="price">
					...
					<?php
					// Get the order price (remove currency symbol and format it correctly)
					$order_price_raw = $item['order_price'];
					$order_price = floatval(preg_replace('/[^\d.]/', '', $order_price_raw));
					...
					// Reverse calculate the regular price
					if ($discount_percentage > 0) {
						$calculated_regular_price = $order_price / (1 - ($discount_percentage / 100));
					} else {
						$calculated_regular_price = $order_price;
					}

					// Final output
					if ($discount_percentage > 0):
						echo '<s>' . wc_price($calculated_regular_price) . '</s><br>';
					endif;
					echo $order_price_raw . '<br>';
					if ($discount_percentage > 0):
						echo '<small>(Discount ' . esc_html($discount_percentage) . ' %)</small>';
					endif;
					?>

				</td>
				<!-- <td class="price">
					<?php echo esc_html($item['order_price']); ?>
					<?php if ($discount_percentage > 0): ?>
						<br><small>(Discount <?php echo esc_html($discount_percentage); ?>%)</small>
					<?php endif; ?>
				</td> -->
			</tr>
		<?php endforeach; ?>
					...
					<tfoot>
						<?php foreach ($this->get_woocommerce_totals() as $key => $total): ?>
							<tr class="<?php echo esc_attr($key); ?>">
								...
								<td class="price"><span class="totals-price"><?php echo esc_html($total['value']); ?></span>
								</td>
							</tr>
						<?php endforeach; ?>
					</tfoot>
