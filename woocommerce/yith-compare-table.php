				...
				<tr class="remove">
					...
					foreach ( $products as $product_id => $product ) :
						...
						<td class="<?php echo esc_attr( $product_class ); ?>">
							<a class="remove-compare-item" href="<?php echo esc_url( $yith_woocompare->obj->remove_product_url( $product_id ) ); ?>"
				...
				<tr class="<?php echo ! in_array( $field, $different, true ) ? esc_attr( $field ) : esc_attr( $field ) . ' different'; ?>">
					...
					<th>
						...
						if ( 'product_info' === $field ) {
							echo '<div class="fixed-th"><span class="small">Rows with differences highlighted below</span></div>';
						}
						?>
					</th>
							...
							switch ( $field ) {
								...
								case 'product_info':
									if ( ! $fixed ) {
										 ...
										 * @return string
										 */
										echo '<div class="remove"><a class="remove-compare-item" href="' . esc_url( $yith_woocompare->obj->remove_product_url( $product_id ) ) . '" data-iframe="' . esc_attr( $iframe ) . '" data-product_id="' . esc_attr( $product_id ) . '"><span class="remove">' . wp_kses_post( apply_filters( 'yith_woocompare_remove_icon', 'x' ) ) . '</span>' . wp_kses_post( apply_filters( 'yith_woocompare_remove_label', esc_html__( 'Remove', 'yith-woocommerce-compare' ) ) ) . '</a></div>';
									}
